...
const CartModal = ({ setOpen, acceptingOrders }) => {
				...
				<div className='bg-light rounded-lg max-w-2xl w-full p-6 space-y-6 relative overflow-y-auto max-h-[80vh]'>
					...
					<div className='sticky bottom-0 bg-light pt-4 space-y-4'>
						...
						<button
							...
							className={`w-full py-3 rounded font-black text-xl transition
    ${acceptingOrders ? 'bg-primary text-light hover:bg-dark' : 'bg-primary/30 text-dark/70 italic cursor-not-allowed scale-95'}`}
						>
							{acceptingOrders ? 'Checkout' : 'Currently Unavailable'}
						</button>
