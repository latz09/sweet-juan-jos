		...
		<div className='max-w-6xl mx-auto p-2 lg:p-4'>
			...
			<AnimateUp>
				<div className='max-w-4xl mx-auto border border-primary/60 shadow-primary/20 px-6 pt-8 pb-12 rounded-sm shadow-md bg-primary/10'>
					...
					{data.CMSOwneradditionalDetails &&
						data.CMSOwneradditionalDetails.length > 0 && (
							<div className='grid mt-16'>
								<Notes data={data.CMSOwneradditionalDetails} />
							</div>
						)}
				{
					data.selectedDisplayItems && data.selectedDisplayItems.length > 0 && (
						<div className='grid mt-16'>
							<DisplayItems data={data.selectedDisplayItems} />
						</div>
					)
				}
